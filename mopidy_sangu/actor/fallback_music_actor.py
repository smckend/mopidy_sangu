...
class FallbackMusicActor(pykka.ThreadingActor, CoreListener):
    def __init__(self, config, core):
        ...
        self.shuffle_fallback_playlist = config["sangu"].get("shuffle_fallback_playlist", False)
        self.core = core
    ...
    def tracklist_changed(self):
        ...
        if self.fallback_track_list is None:
            ...
            lookup_result = self.core.library.lookup(uris=[self.fallback_uri]).get()
            ...
            logger.info("Fetched {} track(s).".format(len(self.fallback_track_list)))
        track_list = self.core.tracklist.get_tl_tracks().get()
        if len(track_list) <= 1 and \
                len(self.fallback_track_list) > 0 and \
                self.core.playback.get_state().get() == "playing":
            logger.info("Adding fallback tracks...")
            for i in range(0, 2 - len(track_list)):
                self.core.tracklist.add(uris=[self.fallback_track_list[self.fallback_index].uri]).get()
                logger.info("Added track at index {}.".format(self.fallback_index))
